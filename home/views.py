...
from .models import Profile, Post, LikePost, FollowersCount,Usermode,Products,Recommendation,Cart,participate
...
def index(request):
    ...
    total=len(product_cart_list)
    u_total=len(username_profile_list)
    return render(request, 'index.html', {'product': product_cart_list[:9],'user_profile': user_profile, 'posts':feed_list, 'suggestions_username_profile_list': suggestions_username_profile_list[:4],'user_city':user_city,'total':total,'u_total':u_total})
...
def search(request):
    ...
    return render(request, 'search.html', {'product_cart_list':product_cart_list,'user_profile': user_profile, 'category': category_type_list})
...
def provider(request):
    ...
    return render(request, 'provider.html', {'user_profile': user_profile, 'posts':feed_list, 'suggestions_username_profile_list': suggestions_username_profile_list[:4],'total':total,'u_total':u_total})
...
def create(request):
    ...
    return render(request, 'create.html', {'user_profile': user_profile, 'posts':feed_list, 'suggestions_username_profile_list': suggestions_username_profile_list[:4],'total':total,'u_total':u_total})
...
def participation(request):
    ...
    if request.method == 'POST':
        ...
        new_product =participate.objects.create(user=user, productimg=productimg, description=description,price=price,type=type,name=name,location=location,phone=phone)
        new_product.save()
...
def cart(request):
    ...
    suggestions_username_profile_list = list(chain(*username_profile_list))
    product = participate.objects.all()
    all = participate.objects.values()
    # price = Cart.objects.filter(price=type).values()
    product_cart=[]
    product_id=[]
    ...
    for ids in product_id:
        prolist = participate.objects.filter(id=ids)
        product_cart.append(prolist)
    random.shuffle(product_cart)
    product_cart_list = list(chain(*product_cart))
    user_profile1 = Profile.objects.get(user=request.user)
    ...
    all1 = Products.objects.filter(price='150').values()
    # price = Products.objects.filter(price=type).values()
    product_cart1=[]
    product_id1=[]

    for users in product1:
        product_id1.append(users.id)

    for ids in product_id1:
        prolist = Products.objects.filter(id=ids)
        product_cart1.append(prolist)
    random.shuffle(product_cart1)
    product_cart_list1 = list(chain(*product_cart1))
    return render(request, 'cart.html', {'product1': product_cart_list1,'product': product_cart_list,'user_profile': user_profile, 'posts':feed_list, 'suggestions_username_profile_list': suggestions_username_profile_list,'user_city':user_city,'total':total,'u_total':u_total})
